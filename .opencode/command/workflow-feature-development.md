---
description: Chain Proposal → Planner → Builder → QA → Release → Meta-Agent → PM with todo enforcement and logged skill usage.
---
...
1. **Orchestrator**
   ...
   - Spawn `@proposal` for every new or ambiguous change request, supplying bead/change IDs, user intent, constraints, and success metrics before Planner engages.
   - Log a knowledge-graph entry (`workflow: feature-development`, phase: orchestrator).
2. **Proposal**
   - Read existing context, record missing details as clarifying questions, and draft/refresh `openspec/changes/<id>/` artifacts (proposal, tasks, design, spec) with cited research.
   - Keep all edits scoped to `openspec/`, run `openspec validate <id> --strict`, and log findings to beads + knowledge-graph so Planner inherits the rationale.
   - Hand off a summary (clarifications resolved, open risks, validation output) to Orchestrator and Planner.
3. **Planner**
   - Read Proposal deliverables plus relevant specs, run `openspec list/show` as needed, and draft a numbered plan (<7 steps) citing files/tests.
