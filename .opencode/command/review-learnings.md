...
2. Run `./bin/review-learnings` (interactive) or `./bin/review-learnings --all` (read-only list). The tool scans every ledger entry and highlights those in `new`, `needs-agents-update`, or `needs-spec-change` states.
