...
---
Create a new OpenSpec change proposal. If a change ID is not provided, deduce an appropriate verb-led ID from the context.
...
**Steps**
1. If no change ID is provided, analyze the request and generate a verb-led ID (e.g., `add-dark-mode`, `fix-auth-flow`, `refactor-api-client`).
2. Review `openspec/project.md`, run `openspec list` and `openspec list --specs`, and inspect related code or docs to ground the proposal in current behaviour; note any gaps that require clarification.
3. Scaffold `proposal.md`, `tasks.md`, and `design.md` (when needed) under `openspec/changes/<id>/`.
