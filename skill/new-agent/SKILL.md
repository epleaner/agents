...
- Existing agent definitions in `agents/` (or `.opencode/agents/` when scaffolded)
...
```bash
ls agents/                             # List existing agents
cat agents/<name>.md                   # Review similar agents
```
...
### Agent File: `agents/<name>.md` (or `.opencode/agents/<name>.md` when scaffolded)
