...
- Full-text search (use ripgrep): `rg -n "Requirement:|Scenario:" openspec/specs` (or `.opencode/openspec/specs` when scaffolded)
...
1) Locate the existing requirement in `openspec/specs/<capability>/spec.md` (or `.opencode/openspec/specs/<capability>/spec.md` when scaffolded).
...
# (or use `.opencode/openspec/...` when scaffolded)
