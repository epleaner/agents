...
- **PM**: GPT-5.1 Codex (temperature <=0.25) with limited `write` (docs/AGENTS) and `bash` (bd, openspec, jira) permissions, skills `jira`, `linear-sync`, `slack-notify`, `fathom-notes`. Maintains beads/OpenSpec linkage and external comms (Slack/Jira/Linear) referencing change IDs.
...
- **Writer**: GPT-5.1 Codex or Gemini Flash (temperature <=0.28) with `write` limited to markdown/docs, `bash`: deny, skills `slack-notify`, `jira`. Drafts release notes, spec deltas, and external summaries referencing relevant beads/change IDs.
